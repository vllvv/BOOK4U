...
function deletereservbtn(btnid, i){
	  var gg = confirm('예약을 취소하겠습니까?');
	  if (gg==true) {
		  location.href = 'deleteReserv.jsp?reservID='+ btnid +'&num=' + i ;
	  }
	  else {
			...
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd"  >
				...
				<tbody>
					...
					<tr>
						...
						<td><a href='lodgingDetails.jsp?id=<%= rlist.get(i).getid() %>&img=<%= lodging_info.get(rlist.get(i).getid()-1).getImg() %>'><%= rlist.get(i).gettitle() %></a></td>
						<td><%= rlist.get(i).getprice() %></td>
						<td><%= rlist.get(i).getcheckin()%></td>
						<td><%= rlist.get(i).getcheckout()%></td>
						<td><button class="btn btn-primary pull-left" onclick="deletereservbtn('<%= rlist.get(i).getreservID() %>', '<%=i%>')">취소</button></td>
					</tr>
					<%
						}
					%>
