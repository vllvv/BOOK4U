...
%>
   ...
   <header class="masthead bg-light" style="padding-bottom:0;">
      ...
      <form method="post" id="search" name="search" action="searchLodging.jsp">
      <table class="pull-right" style="position:absolute;top:430px;right:165px">
         <tr>
         ...
         <div class="container bg-light">
            ...
            <div style="margin-right: 58px;" align="right" class="select-container" >
			    <select class="form-control" name="sorting" style="width: 140px; max-width: 100%; min-width: 140px;" onchange="handleSortingChange(this)">
			      <option value="title_desc" <%= (orderBy + "_" + orderDir).equals("title_desc") ? "selected=\"selected\"" : "" %>>이름 (내림차순)</option>
				<option value="title_asc" <%= (orderBy + "_" + orderDir).equals("title_asc") ? "selected=\"selected\"" : "" %>>이름 (오름차순)</option>
				<option value="price_desc" <%= (orderBy + "_" + orderDir).equals("price_desc") ? "selected=\"selected\"" : "" %>>가격 (내림차순)</option>
				<option value="price_asc" <%= (orderBy + "_" + orderDir).equals("price_asc") ? "selected=\"selected\"" : "" %>>가격 (오름차순)</option>
				<option value="location_desc" <%= (orderBy + "_" + orderDir).equals("location_desc") ? "selected=\"selected\"" : "" %>>장소 (내림차순)</option>
				<option value="location_asc" <%= (orderBy + "_" + orderDir).equals("location_asc") ? "selected=\"selected\"" : "" %>>장소 (오름차순)</option>
			    </select>
			</div>
            ...
            <div class="row">
               <div class="col-lg-4 col-sm-6 mb-4">
                  ...
                  <div class="portfolio-item"  style="background-color: none;">
                     <a href="lodgingDetails.jsp?id=<%= lodging.getId() %>&img=<%= lodging.getImg() %>">
                        <img src="data:image/jpeg;base64,<%= urlConnection(lodging.getImg()) %>" alt="Lodging Image" style="width: 300px; height: 200px;">
                     </a>
