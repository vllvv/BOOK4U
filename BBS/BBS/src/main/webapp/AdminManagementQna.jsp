...
<section class="page-section bg-none" id="portfolio">
    <div class="container">
        <div class="row">
            ...
            <nav class="navbar bg-body-tertiary">
			  ...
			  <div class="container-fluid center-div">
			    <ul class="nav justify-content-end">
				  ...
				  <li class="nav-item">
				    <a class="nav-link active" href="AdminManagementQna.jsp">QnA 관리</a>
				  </li>
				  <li class="nav-item">
            ...
            <form id="Management" action="AdminDeletebbsAction.jsp" method="post" onsubmit="return validateForm();" class="admin-form">
                <table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
                    <thead>
                    
                        <tr>
                            <th colspan="6" style="background-color: #eeeeee; text-align: center;">QNA 관리</th>
                        </tr>
                        <!-- --------------------------			검색 및 정렬		---------------------------------- -->
                    	<tr>
                            <td>
							<div style="margin-right: 58px;" align="right" class="select-container" >
							    <select class="form-control" name="sorting" style="width: 160px; max-width: 100%; min-width: 160px;" onchange="handleSortingChange(this)">
							    	<option value="bbsID_desc" <%= (orderBy + "_" + orderDir).equals("bbsID_desc") ? "selected=\"selected\"" : "" %>>번호 (내림차순)</option>
									<option value="bbsID_asc" <%= (orderBy + "_" + orderDir).equals("bbsID_asc") ? "selected=\"selected\"" : "" %>>번호 (오름차순)</option>
									<option value="userID_desc" <%= (orderBy + "_" + orderDir).equals("userID_desc") ? "selected=\"selected\"" : "" %>>작성자 (내림차순)</option>
									<option value="userID_asc" <%= (orderBy + "_" + orderDir).equals("userID_asc") ? "selected=\"selected\"" : "" %>>작성자 (오름차순)</option>
									<option value="bbsTitle_desc" <%= (orderBy + "_" + orderDir).equals("bbsTitle_desc") ? "selected=\"selected\"" : "" %>>제목 (내림차순)</option>
									<option value="bbsTitle_asc" <%= (orderBy + "_" + orderDir).equals("bbsTitle_asc") ? "selected=\"selected\"" : "" %>>제목 (오름차순)</option>
							    </select>
							</div>
							...
							<script>
							    function handleSortingChange(select) {
							        ...
							        location.href = 'AdminManagementQna.jsp?orderBy=' + selectedValue.split('_')[0] + '&orderDir=' + selectedValue.split('_')[1]+'&formName=Management';
							    }
							</script>
							</td>
							...
							<td><select class="form-control" id="searchFiled" name="searchField"  style="width:80px;" >
				            ...
				            <script>
							    function searching() {
							        ...
							        location.href = 'AdminManagementQna.jsp?searchField=' + searchField + '&searchText=' + searchText+'&formName=Management';
							    }
							</script>
							</tr>
                    ...
                    <tbody align="center" >
						...
						if (searchText=="") {
							...
							BbsDAO bbsdao2 = new BbsDAO(); // 페이징을 위한 sql
							total = bbsdao2.getMax(searchField, searchText); // 검색한 모든 데이터
							numOfPages = (int) Math.ceil((double) total / usersPerPage);
							
							ArrayList<Bbs> bbslist = bbsdao.getAllLists(orderBy, orderDir, usersPerPage, start);
							for (int i = 0; i< bbslist.size(); i++){
								%>
								<tr>
				                  	<td>
		                                <input type="checkbox" name="checkId" value="<%=bbslist.get(i).getBbsID()%>">
		                            </td>
				                  	
				                  	<td> <%=bbslist.get(i).getBbsID() %></td>
				                  	<td><a style="text-decoration: none;" href="view.jsp?bbsID=<%=bbslist.get(i).getBbsID()%>"><%=bbslist.get(i).getBbsTitle()%></td>
				                  	<td><%=bbslist.get(i).getUserID() %></td>			              
				                  	<td> <%=bbslist.get(i).getFileName() %></td>
							...
							ArrayList<Bbs> bbslist = bbsdao.getSearchAllList(searchField, searchText, orderBy, orderDir, usersPerPage, start);
                ...
                <table align="center">
                    ...
                    <tr align="center">
                    	<td><input type="submit" class="btn btn-primary pull-right" value="QnA 삭제"></td>
                    </tr>
                </table>
            </form>
